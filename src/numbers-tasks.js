...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const sq = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(sum / sq);
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const numberPrev = numbers[i - 2];
    const numberNext = numbers[i - 1];
    numbers.push(numberPrev + numberNext);
  }
